...
const inputSchema = z.object({
  ...
  since: z
    .string()
    .describe('ISO 8601 timestamp. Returns ledger entries with created_at >= this value.'),
  limit: z
    .number()
    .int()
    .min(1)
...
export const getLedgerEntries: ToolDefinition<Args, Result> = {
  ...
  description:
    'Return audit-immutable Monthly Reading rows for a workload since a given timestamp. Each entry records the provider call (e.g. openai/gpt-4o-mini-2024-07-18), the mechanic stack applied (e.g. ["M1", "M3", "M5"] for auto-route + auto-compress + auto-cache), anchor cost (baseline pricing before Tessera optimization), actual cost (post-optimization), and the delta savings in USD. Anchor cost references the pricing-catalog snapshot captured at the moment of the call — pricing changes after the fact never alter past entries.',
  inputSchema,
